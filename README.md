...
will generate 12 databases (and indexes) of different sizes. The generation process is resource intensive and may take several days for all databases.
...
The databases are based on and compatible with [Genestrip v2.4](https://github.com/pfeiferd/genestrip/releases/tag/v2.4).
...
| `fungal_infect`    | `fungi`        | Fungal infections of humans from the [RefSeq](https://ftp.ncbi.nlm.nih.gov/refseq/release/) and [Genbank](https://ftp.ncbi.nlm.nih.gov/genomes/genbank/)                                                                                             | N/A             | Collected from the [CDC pages on fungal diseases](https://www.cdc.gov/fungal/about/types-of-fungal-diseases.html) |


Note that Genestrip's [`updatedb`](https://github.com/pfeiferd/genestrip/blob/master/Goals.md)-phase accounts for unspecific *k*-mers and largely avoids false positive counts during `match`es.
...
## Testing the databases `borrelia`, `borrelia_plasmid` and `tick-borne`
...
to the [`projects` folder's](https://github.com/pfeiferd/genestrip-db/tree/master/data/projects) state of this project, after the script `bin/makedbs.sh` has run successfully based on the [RefSeq Release 232](https://ftp.ncbi.nlm.nih.gov/refseq/release/RELEASE_NUMBER).
